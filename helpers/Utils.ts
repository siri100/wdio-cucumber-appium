import {CONTEXT_REF} from "./WebView.js";
...
export async function dismissKeyboard() {
    if (driver.isIOS) {
        ...
        if (currentContext === CONTEXT_REF.NATIVE_APP) {
            await $('~Done').click();
        } else {
            await driver.switchContext(CONTEXT_REF.NATIVE_APP);
            await $('~Done').click();
            await driver.switchContext(currentContext.toString())
        }
...
export async function isAUTAnApp() {
    return driver.capabilities['app'] != undefined;
}
export async function isAUTAndroidChrome() {
    return driver.capabilities['platformName'] === 'Android' && driver.capabilities['browserName'] === 'chrome'
}
