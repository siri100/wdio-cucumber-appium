...
class WebView {
     ...
     * Custom implementation to switch to a webview for Android and iOS
     */
    async switchBackToChromeContext() {
        await driver.switchContext(CONTEXT_REF.CHROMIUM);
    }

    /**
     * Custom implementation to switch to a webview for Android and iOS
     */
    async switchBackToSafariContext() {
        let contexts: string[] = (await driver.getContexts()).map(String);
        let matchingContext: string = contexts.find(context => context.includes(CONTEXT_REF.WEBVIEW)) || CONTEXT_REF.WEBVIEW;
        await driver.switchContext(matchingContext);
    }
     ...
     */
    private findMatchingContext({contexts, identifier, title, url}: {
        contexts: CrossPlatformContexts;
        identifier: string;
        title?: string;
        url?: string;
    }) {
        return contexts.find(context => {
            ...
            return idMatch && titleMatches && urlMatches;
        });
    }
     ...
     * NOTE: This is an internal method and should not be called outside of this class
     */
    private generateNonMatchingErrorMessage({identifier, title, url}: {
        identifier: string,
        title?: string,
        url?: string
    }): string {
    ...
    async waitForWebsiteLoaded() {
        ...
        await this.switchToContext({context: CONTEXT_REF.WEBVIEW, title: 'JDW'});
        await this.waitForDocumentFullyLoaded();
